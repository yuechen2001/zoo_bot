...
async def visit_feed(uid: int = Depends(get_uid)):
    ...
    if last:
        ...
        if elapsed < VISIT_FEED_COOLDOWN_HOURS * 3600:
            ...
            raise HTTPException(
                status_code=400, detail=f"Cooldown: {h}h {m}m remaining"
            )
    ...
    if user["coins"] < cost:
        raise HTTPException(
            status_code=400, detail=f"Not enough coins (need {cost} 🪙)"
        )
...
async def gift_animal(body: GiftBody, uid: int = Depends(get_uid)):
    ...
    if db.has_pending_trade_for_animal(body.animal_id):
        raise HTTPException(
            status_code=400, detail="Animal has a pending trade — cancel it first"
        )
    ...
    if current >= capacity:
        raise HTTPException(
            status_code=400, detail=f"Partner's {habitat} enclosure is full"
        )
    ...
    return {
        "success": True,
        "message": f"🎁 {animal['emoji']} {name} gifted to @{partner['username']}!",
    }
...
async def list_trades(uid: int = Depends(get_uid)):
    ...
    for row in rows:
        my_animal_id = (
            row["proposer_animal_id"]
            if row["proposer_id"] == uid
            else row["recipient_animal_id"]
        )
        their_animal_id = (
            row["recipient_animal_id"]
            if row["proposer_id"] == uid
            else row["proposer_animal_id"]
        )
...
async def propose_trade(body: ProposeTradeBody, uid: int = Depends(get_uid)):
    ...
    if db.has_pending_trade_for_animal(body.my_animal_id):
        raise HTTPException(
            status_code=400, detail="Your animal already has a pending trade"
        )
    if db.has_pending_trade_for_animal(body.their_animal_id):
        raise HTTPException(
            status_code=400, detail="Their animal already has a pending trade"
        )

    trade_id = db.create_trade(
        uid, partner["user_id"], body.my_animal_id, body.their_animal_id
    )
...
async def respond_trade(
    trade_id: int, body: TradeRespondBody, uid: int = Depends(get_uid)
):
    ...
    if body.action == "accept":
        ...
        if p_habitat != r_habitat:
            ...
            cap = ENCLOSURE_LEVELS[db.get_enclosure_level(proposer_id, r_habitat)][
                "capacity"
            ]
            if used >= cap:
                raise HTTPException(
                    status_code=400,
                    detail="Proposer's enclosure is full for that habitat",
                )
            used = db.get_animal_count_by_habitat(uid, p_habitat)
            cap = ENCLOSURE_LEVELS[db.get_enclosure_level(uid, p_habitat)]["capacity"]
            if used >= cap:
                raise HTTPException(
                    status_code=400, detail="Your enclosure is full for that habitat"
                )
        ...
        return {
            "success": True,
            "message": f"✅ Trade complete! {p_animal['emoji']} {p_name} ↔ {r_animal['emoji']} {r_name}",
        }
    else:
        db.resolve_trade(trade_id, "declined")
